...
class ParserTestSuite extends munit.FunSuite {
	test("Qualified identifier test") {
			val qualTest1 = qualifiedNameP.parse("myModule::myIdentName123 bla blabla asdna")
			qualTest1 match {
				case Right((_, parsed)) => assertEquals(parsed, "myModule::myIdentName123")
				case _ => fail("Qualified identifier incorrectly parsed.")
			}

			val qualTest2 = qualifiedNameP.parse("1myModule::myIdentName123 bla blabla asdna")
			qualTest2 match {
	...
	test("String test") {
		val stringTest1 = quoteStringP.parse("\"uma string de teste\"abcdef")
		stringTest1 match {
